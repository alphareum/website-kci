...
async function ensureDbFile(): Promise<void> {
  try {
    ...
    const initialData: CompanyProfilesDB = { nextId: 1, companyProfiles: [] };
    await fs.writeFile(DB_FILE, JSON.stringify(initialData, null, 2), 'utf-8');
  }
}

async function readDb(): Promise<CompanyProfilesDB> {
  await ensureDbFile();
  const raw = await fs.readFile(DB_FILE, 'utf-8');
  return JSON.parse(raw);
}

async function writeDb(db: CompanyProfilesDB): Promise<void> {
 ...
 */
export async function getCompanyProfileById(id: number): Promise<CompanyProfile | null> {
  const db = await readDb();
 ...
 */
export async function createCompanyProfile(input: UpsertCompanyProfileInput): Promise<CompanyProfile> {
  const db = await readDb();
  ...
  const newProfile: CompanyProfileRecord = {
    id: db.nextId,
    ...input,
  };
...
export async function updateCompanyProfile(
  ...
  const slugConflict = db.companyProfiles.find(
    (p) => p.slug === input.slug && p.id !== id
  );
  if (slugConflict) {
    throw new Error(`Company profile with slug "${input.slug}" already exists`);
  }

  const updatedProfile: CompanyProfileRecord = {
    id,
    ...input,
  };
