...
function buildContactRecord(id, input, existing) {
    return {
        id,
        ...
        whatsapp_url: typeof input.whatsapp_url === 'string'
            ? input.whatsapp_url
...
export class ContactsService {
    async listContacts() {
        const contacts = await readTable(TABLE, FALLBACK_CONTACTS);
    ...
    async upsertContact(input) {
        ...
        if (payload.id) {
            ...
            const nextContacts = contacts.map((contact) => contact.id === updated.id ? updated : contact);
